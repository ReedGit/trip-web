...
public class CollectionController {
    ...
    public JSONObject isCollected(@RequestParam Map<String, Object> map){
        ...
        boolean isCollect = collectionService.isCollectedByUser(travelId, userId);
        if(isCollect){
            result.put("status", "0");
            result.put("msg", "");
    ...
    public JSONObject getTravels(@RequestParam Map<String, Object> map){
        ...
        PageBean<Travel> travels = collectionService.findCollectionByUser(userId, page, Constants.PAGE_SIZE);
        result.put("status", "0");
